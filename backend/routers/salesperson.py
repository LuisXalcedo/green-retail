...
async def update_salesperson_by_id(
    ...
    try:
        ...
        if not salesperson:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Salesperson not found"
            )
...
@router.get(
    ...
    response_model=int,
    status_code=status.HTTP_200_OK,
    summary="Get the number of salespersons",
)
...
async def get_salesperson_by_id(
    id: ObjectId, current_user: Salesperson = Depends(get_current_active_user)
):
    """
...
@router.delete(
    "/{id}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete a salesperson"
)
